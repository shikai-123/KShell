...
void HomePage::slotDeletOrderDBtoUserList()
{
	...
	if (item->parent() == nullptr)								  // 点击父节点
	{
		...
		for (int ChildIndex = 0; ChildIndex < item->childCount(); ChildIndex++)
		{
			...
			for (int GrandChildIndex = 0; GrandChildIndex < ChildItem->childCount(); GrandChildIndex++)
			{
				...
				QString CJQuuid = GrandChildItem->parent()->parent()->text(0) + GrandChildItem->parent()->text(0) + GrandChildItem->text(1) + GrandChildItem->text(2) + GrandChildItem->text(3) + GrandChildItem->text(4) + GrandChildItem->text(5) + GrandChildItem->text(6) + GrandChildItem->text(7) + GrandChildItem->text(8) + GrandChildItem->text(9) + GrandChildItem->text(10);
	...
	else // 单击子节点，在该项目中没有什么要处理的，所以就不做处理
	{
		...
		{
			QTreeWidgetItem *ChildItem = item->child(ChildIndex);
			QString CJQuuid = ChildItem->parent()->parent()->text(0) + ChildItem->parent()->text(0) + ChildItem->text(1) + ChildItem->text(2) + ChildItem->text(3) + ChildItem->text(4) + ChildItem->text(5) + ChildItem->text(6) + ChildItem->text(7) + ChildItem->text(8) + ChildItem->text(9) + ChildItem->text(10);
			m_ProjectCJQName[CJQuuid] = nullptr;
			qDebug() << CJQuuid;
		}
