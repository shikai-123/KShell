...
extern QString g_ProjectRegion, g_ElecRoomID, g_DevIndex, g_DevIP, g_N2NIP, g_DevPort, g_UserName, g_UserWord, g_UserListNote, g_FTPType, g_ProjectName;
...
void NewUser::slotFillDate()
{
	...
	ui.cb_FTPType->setCurrentText(g_FTPType);
...
void NewUser::slotCreatProjectToDB()
{
	if (g_WorM == 0)
	{
		...
		QString No1ID = ui.le_No1ID->text();
		QString No2ID = ui.le_No2ID->text();
		...
		QString FTPType = ui.cb_FTPType->currentText();
		if (ProjectRegion == "" || ProjectName == "" || No1ID == "" || No2ID == "" || UserName == "" || PassWord == "" || IP == "" || Port == "" || Note == "" || FTPType == "")
		{
		...
		//query.exec("insert into UserList (ProjectName, No1ID, No2ID,IP,Port,UserName,PassWord,FTPType,Note) values('测试','1','1','192.168.6.38','22','root','nk5730','集智达','我的备注')");
		//ProjectName, No1ID, No2ID,IP,Port,UserName,PassWord,FTPType,Note
		...
		while (query.next())
		{
			...
			if (ProjectRegion == query.value(0) && ProjectName == query.value(1) && No1ID == query.value(2) && No2ID == query.value(3) && IP == query.value(4) && N2NIP == query.value(5) && Port == query.value(6)\
				&& UserName == query.value(7) && PassWord == query.value(8) && FTPType == query.value(9) && Note == query.value(10))
			{
		...
		query.prepare("insert into UserList (ProjectRegion, ProjectName, No1ID, No2ID,IP,N2NIP,Port,UserName,PassWord,FTPType,Note) values(?,?,?,?,?,?,?,?,?,?,?)");//位置绑定的方式--插入数据
		query.addBindValue(ProjectRegion);
		query.addBindValue(ProjectName);
		query.addBindValue(No1ID);
		query.addBindValue(No2ID);
		query.addBindValue(IP);
		query.addBindValue(N2NIP);
		query.addBindValue(Port);
		query.addBindValue(UserName);
		query.addBindValue(PassWord);
		query.addBindValue(FTPType);
		query.addBindValue(Note);
		query.exec();
		g_UserListDB.close();
		QSqlDatabase::removeDatabase("QSQLITE");
		sigAddUserItem(ProjectRegion, ProjectName, No1ID, No2ID, IP, N2NIP, Port, UserName, PassWord, FTPType, Note);
...
void NewUser::soltDeletProjectFromDB()
{
	...
	{
		query.prepare("delete from UserList where ProjectRegion =? and ProjectName=? and  No1ID=? and  No2ID=? and  IP=? and  N2NIP=? and Port=?	and UserName=?	and PassWord=?	and FTPType=?	and Note=?"); //名称绑定的方式 注意和一般的sql语句不同的是，在text类型的时候他不需要加 ''
...
void NewUser::soltModifProjectFromDB()
{
	if (g_WorM == 1)
	{
		...
		{
			query.prepare("update UserList  set No1ID = ?, No2ID = ?, IP = ?,N2NIP = ?, Port = ?, UserName = ?, PassWord = ?, FTPType = ?, Note = ? where  ProjectRegion = ?  and ProjectName=? and  No1ID=? and  No2ID=? and  IP=?  and  N2NIP=? and Port=?	and UserName=?	and PassWord=?	and FTPType=?	and Note=?"); //名称绑定的方式 注意和一般的sql语句不同的是，在text类型的时候他不需要加 ''
		...
		emit sigModifed(ui.cb_ProjectRegion->currentText(), ui.le_ProjectName->text(), ui.le_No1ID->text(), ui.le_No2ID->text(), ui.le_IP->text(), \
			...
			g_ProjectRegion, g_ProjectName, g_ElecRoomID, g_DevIndex, g_DevIP, g_N2NIP, g_DevPort, g_UserName, g_UserWord, g_FTPType, g_UserListNote
		);
	}
	return;
}
