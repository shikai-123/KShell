...
/*发送原始字节数据*/
void CConnectionForSshClient::slotSendRawData(QByteArray data, int SSHIndex)
{
	Q_UNUSED(SSHIndex);
	...
	{
		qDebug() << "CConnectionForSshClient::slotSendRawData 发送失败 未建立连接:" << getIpPort();
	}
}
...
void CConnectionForSshClient::slotConnected()
{
	...
	m_shell->addToEnvironment("LANG", "en_US.UTF-8");
	m_shell->addToEnvironment("LC_ALL", "en_US.UTF-8");
	m_shell->requestTerminal(QSsh::SshPseudoTerminal("xterm-256color", m_termRows, m_termCols));
	qDebug() << "[SSH] 连接成功，请求终端: xterm-256color" << m_termCols << "x" << m_termRows;
	...
	connect(m_shell.data(), &QSsh::SshRemoteProcess::readyReadStandardError, [&]()
			{
		QByteArray err = m_shell->readAllStandardError();
		qCritical() << "[SSH] Shell stderr:" << getIpPort() << err.left(200); });

	connect(m_shell.get(), &QSsh::SshRemoteProcess::readyReadStandardOutput, [&]()
			{
		QByteArray byteRecv = m_shell->readAllStandardOutput();
		if (!byteRecv.isEmpty()) {
			qDebug() << "[SSH] 收到数据" << byteRecv.size() << "字节:" << byteRecv.toHex().left(200);
			emit sigRawDataArrived(byteRecv, m_SSHIndex);
			QString strRecv = QString::fromUtf8(byteRecv);
			if (!strRecv.isEmpty())
				emit sigDataArrived(strRecv, m_strIp, m_nPort, m_SSHIndex);
		} });
